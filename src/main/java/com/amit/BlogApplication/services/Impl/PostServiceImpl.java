...
public class PostServiceImpl implements PostService {
    ...
    private PostDto postToDto(Posts post) {
        log.info("Mapping post to postDto");
        PostDto dto = this.modelMapper.map(post, PostDto.class);

        // ✅ IMPORTANT: map nested user roles correctly
        if (post.getUsers() != null) {
            UserDto userDto = this.modelMapper.map(post.getUsers(), UserDto.class);

            userDto.setRoles(
                    post.getUsers().getRoles().stream()
                            .map(r -> r.getName())
    ...
    public PostDto updatePosts(PostDto postDto, Integer postId) {
        ...
        log.info("Post updated in updatePost");
